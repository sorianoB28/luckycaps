export type Language = "EN" | "ES";

type Vars = Record<string, string | number | boolean | null | undefined>;

interface Dictionary {
  [key: string]: string | Dictionary;
}

const en: Dictionary = {
  nav: {
    home: "Home",
    shop: "Shop",
    story: "Story",
    account: "Account",
    admin: "Admin",
  },
  langToggle: {
    en: "EN",
    es: "ES",
  },
  brand: {
    name: "Lucky Caps",
  },
  common: {
    loading: "Loading...",
    saving: "Saving...",
    back: "Back",
    backToShop: "Back to shop",
    close: "Close",
    save: "Save",
    cancel: "Cancel",
    clear: "Clear",
    submitting: "Submitting...",
    add: "Add",
    remove: "Remove",
    uploaded: "Uploaded",
    failed: "Failed",
    delete: "Delete",
    submit: "Submit",
    view: "View",
    manage: "Manage",
    search: "Search",
    status: "Status",
    actions: "Actions",
    code: "Code",
    value: "Value",
    enable: "Enable",
    disable: "Disable",
    created: "Created",
    paid: "Paid",
    shipped: "Shipped",
    delivered: "Delivered",
    cancelled: "Cancelled",
    refunded: "Refunded",
    none: "None",
    customer: "Customer",
    type: "Type",
    order: "Order",
    items: "Items",
    subtotal: "Subtotal",
    discount: "Discount",
    total: "Total",
    notes: "Notes",
    tracking: "Tracking",
    upload: "Upload",
    uploading: "Uploading...",
    loadMore: "Load more",
  },
  header: {
    logoAlt: "Lucky Caps logo",
    accountMenu: "Account menu",
    openCart: "Open cart",
    openMenu: "Open menu",
    closeMenu: "Close menu",
    menuTitle: "Menu",
    switchToEnglish: "Switch to English",
    switchToSpanish: "Switch to Spanish",
  },
  footer: {
    tagline: "Lucky Supply",
    blurb:
      "Badge-style caps handcrafted in Des Moines with raised emblems and street-level attitude. Limited drops only, made for people who wear their own identity.",
    shop: "Shop",
    stayInLoop: "Stay in the loop",
    noSpam: "No spam. Just drops and behind-the-scenes.",
    subscribed: "You're in. Check your inbox soon.",
    subscribeError: "We couldn't save your signup. Try again.",
    emailError: "Enter a valid email.",
    consentError: "Please confirm marketing consent.",
    company: "Company",
    legal: "Legal",
    backToTop: "Back to top",
    emailPlaceholder: "you@example.com",
    join: "Join",
    consent: "Send me drops & promos",
    copyright: "Ac {year} Lucky Caps. All rights reserved.",
    facts: {
      emblems: "Hand-applied emblems",
      smallBatch: "Small-batch caps",
      local: "Made in Des Moines",
    },
    badges: {
      craftedByHand: "Crafted by hand",
      limitedDrops: "Limited edition drops",
      boldIdentity: "Bold identity",
    },
    shopLinks: {
      snapbacks: "Snapbacks",
      fitted: "Fitted",
      trucker: "Trucker",
      beanies: "Beanies",
      custom: "Custom",
      shopAll: "Shop All",
    },
    companyLinks: {
      story: "Story",
      contact: "Contact",
    },
    legalLinks: {
      privacy: "Privacy",
      terms: "Terms",
    },
  },
  categories: {
    snapbacks: "Snapbacks",
    fitted: "Fitted",
    trucker: "Trucker",
    beanies: "Beanies",
    custom: "Custom",
    dad_hat: "Dad Hat",
    rope: "Rope",
    five_panel: "Five Panel",
    flat_bill: "Flat Bill",
    caps: "Caps",
    gorra: "Cap",
    snapbacks_2pk: "Snapbacks 2-Pack",
  },
  home: {
    heroImageAlt: "Cap with neon green glow",
    heroHeadline: "Wear Your Luck.",
    heroSub:
      "Hand-applied emblem caps made in Des Moines. Badge-style logos, finished by hand - built for people who move different.",
    heroBadge: "Badge-style caps",
    heroCtaPrimary: "Shop Collection",
    heroCtaSecondary: "Meet the Founder",
    marquee: {
      emblems: "Hand-applied emblems",
      badgeStyle: "Badge-style logos",
      smallBatch: "Small-batch builds",
      desMoines: "Made in Des Moines",
    },
    newDrops: "Featured Caps",
    freshHeading: "Badge-style caps finished by hand",
    explore: "Explore",
    emptyNewDropsTitle: "New items coming soon.",
    emptyNewDropsCopy: "Check back soon or explore the full shop.",
    featuredCategories: "Featured Categories",
    builtForEveryDrop: "Built for every drop",
    featuredCopy: "Badge-ready picks for every style.",
    viewAll: "View all",
    shopCategoryAria: "Shop category: {category}",
    shopNow: "Shop now",
    emptyCategories: "No categories available yet.",
    midBand: {
      one: "Hand-applied emblems",
      two: "Small-batch drops",
      three: "Made in Des Moines",
    },
    diff: {
      kicker: "What makes us different",
      title: "What makes Lucky Caps different",
      copy: "Real caps built by a barber-turned-maker. Badge-style emblems, hand applied.",
      one: {
        title: "Hand-Applied Emblems",
        copy: "Badge-style logos applied with specialty adhesive for a clean, raised look.",
      },
      two: {
        title: "Small-Batch Builds",
        copy: "Designed and assembled in limited runs to keep each drop special.",
      },
      three: {
        title: "Local Craft",
        copy: "Made by a barber-turned-maker in Des Moines, Iowa.",
      },
    },
    founder: {
      kicker: "The founder",
      title: "Built by Jesus Perez",
      copy:
        "A Des Moines barber who started making caps in Nov 2024 - turning badge-style logos into wearable statements.",
      cta: "Read the story",
      imageAlt: "Portrait of Jesus Perez wearing a Lucky Cap",
    },
    luckySocial: "Lucky Social",
    seenOn: "Seen on the founders",
    socialImageAlt: "Lucky Caps social gallery",
  },
  shop: {
    title: "Shop",
    heading: "Premium Drops",
    kicker: "Shop Collection",
    description: "Filter by category, price, and drop status to find your next cap.",
    itemsCount: "{count} items",
    reset: "Reset",
    refine: "Refine",
    filters: "Filters",
    sort: "Sort",
    categories: "Categories",
    noCategories: "No categories available.",
    priceRange: "Price range",
    highlights: "Highlights",
    newDropsFilter: "New drops",
    onSale: "On sale",
    showing: "Showing {shown} of {total} products",
    featured: "Featured",
    newest: "Newest",
    priceLowHigh: "Price: Low to High",
    priceHighLow: "Price: High to Low",
    upTo: "Up to",
    productsSuffix: "products",
    newDrop: "New Drop",
    sale: "Sale",
    loadingProducts: "Loading products...",
    loadFailed: "Couldn't load products. Please try again.",
    emptyTitle: "No products match your filters.",
    emptyCopy: "Adjust filters or reset to see everything.",
    clearFilters: "Clear filters",
    retryLoad: "Retry",
  },
  product: {
    variant: "Variant",
    size: "Size",
    features: "Features",
    fastShipping: "Fast shipping",
    fastShippingCopy: "Dispatch within 2 business days.",
    returns: "30 Day Returns",
    secureCheckout: "Secure Checkout",
    secureCheckoutCopy: "Encrypted payments for peace of mind.",
    easyReturns: "Easy returns",
    easyReturnsCopy: "30-day hassle-free returns.",
    trust: {
      title: "Built for trust",
      secureCheckout: "Secure checkout powered by Stripe",
      trackedShipping: "Tracked shipping on every order",
      support: "Support: {email}",
      returns: "Returns policy available at checkout",
    },
    decreaseQuantityAria: "Decrease quantity",
    increaseQuantityAria: "Increase quantity",
    addToCart: "Add to Cart",
    outOfStock: "Out of stock",
    addToWishlistAria: "Add to wishlist",
    productNotFound: "Product not found.",
    unableToLoadProduct: "Unable to load product.",
    tryOtherProducts: "Try browsing other products.",
    reportReviewAria: "Report review",
    galleryImageAria: "View {name} image {index}",
  },
  cart: {
    title: "Your Cart",
    empty: "Your cart is empty.",
    emptyCopy: "Browse the shop and add your first drop.",
    emptyPageTitle: "Your cart is empty",
    emptyPageCopy: "Start a new drop and add premium Lucky Caps to your cart.",
    browseCollection: "Browse collection",
    noImage: "No image",
    variantFallback: "Standard",
    sizeFallback: "One size",
    viewCart: "View cart",
    checkout: "Checkout",
    orderSummary: "Order Summary",
    shipping: "Shipping",
    tax: "Tax",
    calculatedAtCheckout: "Calculated at checkout",
    continueShopping: "Continue shopping",
    removeItemAria: "Remove item",
  },
  checkout: {
    title: "Checkout",
    goToAdmin: "Go to Admin",
    heading: "Complete your order",
    autofill: "Autofill demo data",
    notice: "Secure UI flow - payments not enabled yet.",
    loadingCheckout: "Loading checkout...",
    unableToPlaceOrder: "Unable to place order.",
    quoteLoadFailed: "Couldn't calculate shipping or total. Please try again.",
    retryQuote: "Retry totals",
    requiredFieldsError: "Please complete required fields and add items to your cart.",
    missingCheckoutUrl: "Missing checkout url.",
    shippingAddressTitle: "Shipping address",
    emptyTitle: "Your cart is empty.",
    emptyCopy: "Add products before checking out.",
    canceled: "Checkout canceled. You can try again or update your details below.",
    redirectingTitle: "Redirecting to payment",
    redirectingCopy: "Sending you to Stripe to securely complete payment.",
    finalizingTitle: "Finalizing your order",
    finalizingCopy: "Confirming payment and creating your order.",
    unableToFinalize: "Unable to finalize order.",
    retryFinalization: "Retry finalization",
    returnToCheckout: "Return to checkout",
    orderIdLabel: "Order ID",
    processing: "Processing...",
    viewOrder: "View order",
    customer: "Customer",
    email: "Email",
    phone: "Phone",
    phoneOptional: "Phone (optional)",
    shipping: "Shipping",
    contactTitle: "Contact",
    firstName: "First name",
    lastName: "Last name",
    address1: "Address line 1",
    address2: "Address line 2 (optional)",
    city: "City",
    state: "State",
    zip: "ZIP / Postal code",
    country: "Country",
    deliveryMethodTitle: "Delivery method",
    delivery: {
      flat: { name: "Flat Rate Shipping", description: "One simple rate on every order", eta: "3-6 business days" },
    },
    shippingTbdSelect: "TBD - select shipping",
    totalTbd: "TBD",
    taxLabel: "Tax ({rate}%)",
    free: "Free",
    payment: "Payment",
    cardDetails: "Card details",
    orderNotes: "Order notes",
    orderNotesPlaceholder: "Order notes / embroidery instructions",
    orderSummary: "Order summary",
    promoCode: "Promo code",
    promoSoon: "Promo codes coming soon.",
    coupon: "Discount code",
    apply: "Apply",
    couponPlaceholder: "",
    placeOrder: "Proceed to Checkout",
    placingOrder: "Placing order...",
    redirectNotice: "You'll be redirected to Stripe to pay securely.",
    shippingRequiredInline: "Select a shipping option to continue.",
    shippingRequiredError: "Please select a shipping option.",
    shippingPendingError: "Shipping is still pending. Select a shipping option.",
    promoApplied: "Promo applied",
    promoApply: "Apply",
    promoRemove: "Remove",
    promoError: "Promo code error",
    promoErrors: {
      missing: "Enter a promo code.",
      notFound: "Promo code is invalid.",
      inactive: "Promo code is inactive.",
      notStarted: "Promo code is not active yet.",
      expired: "Promo code has expired.",
      minSubtotal: "Requires a subtotal of ${min}.",
      maxRedemptions: "Promo code has reached its redemption limit.",
      notAvailable: "Promo code not available.",
      invalid: "Unable to apply promo code.",
    },
    trust: {
      title: "Checkout with confidence",
      stripe: "Secure checkout powered by Stripe",
      shipping: "Tracked shipping on every order",
      support: "Support: support@luckycapsshop.com",
      returns: "Returns policy available at checkout",
    },
  },
  order: {
    kicker: "Order",
    title: "Order {id}",
    statusValue: "Status: {status}",
    paymentReceived: "Payment received. We're processing your order.",
    sizeValue: "Size: {size}",
    qtyValue: "Qty: {qty}",
    deliveryValue: "Delivery: {delivery}",
    na: "N/A",
    totalsTitle: "Totals",
  },
  auth: {
    signIn: "Sign in",
    signUp: "Create account",
    signOut: "Sign out",
    forgotPassword: "Reset password",
    signInSubtitle: "Access your Lucky Caps account.",
    signUpSubtitle: "Save your drops, track orders, and stay ahead of releases.",
    forgotSubtitle:
      "We'll email you a reset link if an account exists for this email.",
    emailLabel: "Email",
    passwordLabel: "Password",
    firstNameLabel: "First name",
    lastNameLabel: "Last name",
    password: "Password",
    confirmPassword: "Confirm password",
    confirmPasswordLabel: "Confirm password",
    emailPlaceholder: "you@example.com",
    rememberMe: "Remember me",
    forgotPasswordLink: "Forgot password-",
    continueAsGuest: "Continue as guest",
    googleSoon: "Sign in with Google (coming soon)",
    signingIn: "Signing in...",
    signedIn: "Signed in! Redirecting...",
    noAccount: "Don't have an account-",
    createOne: "Create one",
    alreadyAccount: "Already have an account-",
    minPasswordHint: "Minimum 8 characters.",
    termsAgree: "I agree to the Terms & Privacy",
    marketingOptIn: "Email me product drops and promos",
    unsubscribeCopy: "You can unsubscribe anytime.",
    reset: {
      emailSent: "Email sent",
      sentCopy: "If an account exists for {email}, you'll get a reset link shortly.",
      returnToSignIn: "Return to sign in",
      sendLink: "Send reset link",
      remembered: "Remembered your password-",
    },
    errors: {
      invalidEmail: "Enter a valid email.",
      nameRequired: "Name is required.",
      passwordRequired: "Password is required.",
      passwordTooShort: "Password must be at least 8 characters.",
      passwordsMustMatch: "Passwords must match.",
      mustAgreeTerms: "You must agree to the Terms & Privacy.",
      invalidCredentials: "Invalid email or password.",
      signInFailed: "We couldn't sign you in. Please try again.",
      unableToCreateAccount: "Unable to create account.",
    },
    success: {
      signedIn: "You're in. Redirecting...",
    },
  },
  account: {
    title: "Your Lucky Hub",
    heading: "Account",
    guest: "Guest",
    loadingAccount: "Loading account...",
    welcome: "Welcome, {name}",
    intro: "Manage your profile, orders, and preferences.",
    backToShop: "Back to shop",
    profile: "Profile",
    orders: "Order history",
    addresses: "Addresses",
    settings: "Settings",
    name: "Name",
    email: "Email",
    loadingOrders: "Loading orders...",
    noOrders: "No orders yet",
    noOrdersCopy: "Start a drop to see your order history here.",
    shopCollection: "Shop collection",
    orderNumber: "Order #{id}",
    statusValue: "Status: {status}",
    totalValue: "Total ${total}",
    qty: "Qty {qty}",
    noAddresses: "No addresses saved",
    noAddressesCopy: "Add a shipping address to speed up checkout.",
    addAddressSoon: "Add address (coming soon)",
    newPassword: "New password",
    confirmPasswordLabel: "Confirm password",
    updatePasswordSoon: "Update password (coming soon)",
    signOutTitle: "Sign out",
    signOutCopy: "Clear session and return home.",
    unableToLoadOrders: "Unable to load orders.",
  },
  about: {
    badge: "The Story",
    title: "The Story",
    heroHeading: "Meet Jesus Perez",
    heroSub:
      "Founder of Lucky Caps. Barber in Des Moines. Started in Nov 2024 with a simple idea: turn badge-style emblems into caps that feel like a statement.",
    imageAlt: "Photo of Jesus Perez",
    fact: {
      role: "Founder & CEO, Lucky Caps",
      location: "Barber in Des Moines, Iowa",
      started: "Started November 2024",
    },
    sections: {
      kicker: "Founder notes",
      how: {
        title: "How it started",
        copy:
          "In November 2024, Jesus began making and selling hats locally in Des Moines - building each piece by hand and learning what people actually want to wear.",
      },
      what: {
        title: "What makes a Lucky Cap",
        copy:
          "Each cap features a badge-style emblem applied with specialty adhesive for a clean, raised finish. Built in small batches, checked before it ships.",
      },
      why: {
        title: "Why 'Wear Your Luck'",
        copy:
          "Lucky Caps is about backing yourself - showing up, staying consistent, and letting your style speak.",
      },
    },
    whyTitle: "Wear your luck",
    whyHeadline: "Why Lucky Caps",
    whyCopy:
      "Made for people who move different. Built to feel like a badge you can wear anywhere.",
    ctaShop: "Shop collection",
  },
  contact: {
    title: "Contact",
    copy: "Need help- Drop us a line at support@luckycapsshop.com.",
  },
  legal: {
    lastUpdated: "Last updated {date}",
    toc: "On this page",
    disclaimer:
      "This page shares how Lucky Caps works today. It is not legal advice. Please review with your counsel.",
    policyTbd: "Policy TBD",
    todo: "TODO: confirm details with the team before launch.",
    contact: "Contact us at {email}",
  },
  privacy: {
    title: "Privacy Policy",
    description:
      "How Lucky Caps collects, uses, and shares information to run the store, ship orders, and send updates.",
    sections: {
      overview: {
        title: "Overview",
        body1:
          "This policy explains how {businessName} (\"we\", \"us\") handles personal information when you browse, create an account, or buy from luckycapsshop.com.",
        body2:
          "We use Stripe for payments, Shippo for labels and tracking, Resend for transactional emails, Neon Postgres for our database, and NextAuth for account sessions.",
      },
      infoWeCollect: {
        title: "Information we collect",
        items: {
          account: "Account details if you sign in (email and name you provide via NextAuth).",
          checkout:
            "Order and checkout details- items, shipping address, email, phone (if provided), and chosen delivery options.",
          payment: "Payment confirmations from Stripe (we do not store full card numbers).",
          device:
            "Device and log data such as IP address, browser type, and interaction timestamps for security and fraud prevention.",
          media: "Photos you upload for custom work or reviews (stored via Cloudinary).",
        },
      },
      howWeUse: {
        title: "How we use information",
        bullets: {
          fulfill: "Fulfill and deliver orders, including creating shipping labels with Shippo.",
          communicate:
            "Send order confirmations, shipping updates, and account-related notices via Resend from {replyToEmail}.",
          support: "Provide customer support and respond to questions sent to {supportEmail}.",
          auth: "Authenticate accounts and keep sessions active via NextAuth.",
          improve: "Maintain and improve site performance, prevent fraud/abuse, and protect the service.",
          marketing: "Send marketing emails only if you opt in to \"Stay in the loop\"; you can unsubscribe anytime.",
        },
      },
      marketing: {
        title: "Marketing choices",
        body1:
          "Marketing email signups are optional and separate from transactional emails. Unsubscribe links are included in each marketing email.",
        body2: "You may also email {supportEmail} to be removed. Transactional emails for orders will still arrive as needed.",
      },
      cookies: {
        title: "Cookies & analytics",
        body1: "We use essential cookies for things like keeping your session, cart, and language preference.",
        body2: "Analytics/ads tools are not fully configured yet. TODO- confirm and list any analytics provider before launch.",
        body3: "You can manage cookies through your browser settings; disabling some cookies may affect checkout or login.",
      },
      sharing: {
        title: "How we share information",
        intro: "We share data only with service providers needed to run Lucky Caps-",
        items: {
          stripe: "Stripe for payment processing and fraud prevention.",
          shippo: "Shippo for shipping rates, labels, and tracking.",
          resend: "Resend for sending emails.",
          database: "Neon Postgres (via Netlify) to store orders, accounts, and product data.",
          cloudinary: "Cloudinary to host product and user-uploaded images.",
          hosting: "Netlify/Next.js infrastructure to host the storefront.",
        },
        closing:
          "We do not sell personal information. If we need to share data for legal compliance or to protect the service, we will do so as permitted by law.",
      },
      retention: {
        title: "Data retention",
        body1:
          "We keep order and account records for as long as necessary to operate the store, comply with law, resolve disputes, and maintain tax/financial records.",
        body2:
          "If you request deletion, we will remove or de-identify data we are not required to keep. Some records may remain in backups for a limited period.",
      },
      security: {
        title: "Security",
        body1:
          "We use reasonable technical and organizational measures (access controls, encrypted transport, role-based admin tools) to protect data stored in Neon Postgres and our providers.",
        body2:
          "No method of transmission or storage is 100% secure. If we learn of a security issue that affects you, we will reach out using your contact details.",
      },
      children: {
        title: "Children's privacy",
        body: "Lucky Caps is not directed to children under 13. If you believe a child provided personal information, contact us at {supportEmail} and we will delete it where required.",
      },
      rights: {
        title: "Your rights & choices",
        body1: "You can request access, correction, or deletion of your information by contacting {supportEmail}.",
        body2:
          "If you are in a region with specific privacy rights (e.g., EU/UK/California), let us know in your request so we can apply the appropriate process. TODO- add region-specific language once targeting is confirmed.",
      },
      updates: {
        title: "Updates to this policy",
        body:
          "We may update this policy and will change the \"Last updated\" date when we do. If changes are significant, we will provide a brief notice on the site.",
      },
      contact: {
        title: "Contact us",
        body: "Questions about privacy- Email {supportEmail}.",
      },
    },
  },
  terms: {
    title: "Terms of Service",
    description: "The rules for using Lucky Caps and purchasing our small-batch caps.",
    sections: {
      overview: {
        title: "Overview / Agreement to terms",
        body1: "By accessing or purchasing from Lucky Caps, you agree to these Terms and our Privacy Policy.",
        body2:
          "If you do not agree, please do not use the site. We may update the Terms periodically- continued use after changes means you accept the updated terms.",
      },
      eligibility: {
        title: "Eligibility",
        body1:
          "You must be at least 18 (or the age of majority in your region) or have parental consent to place orders or create an account.",
        body2:
          "You agree to provide accurate information. We may refuse or cancel orders at our discretion (for example, suspected fraud or inventory limits).",
      },
      orders: {
        title: "Orders & payment",
        bullets: {
          payment: "Payments are processed by Stripe. We do not store full card numbers.",
          pricing: "Prices and availability can change. Taxes and shipping are shown at checkout.",
          auth:
            "By submitting an order, you authorize us (and Stripe) to charge your payment method for the order total, including taxes, shipping, and any discounts applied.",
          promos:
            "Promotions or discount codes may be limited by time, usage, or inventory; they cannot be combined unless stated.",
          confirmation:
            "Orders are confirmed when we accept and charge the payment. If we cannot fulfill an order, we will cancel and refund you.",
        },
      },
      shipping: {
        title: "Shipping & delivery",
        body1:
          "We ship using Shippo-generated labels. Shipping options and estimated timelines are shown at checkout; actual delivery depends on the carrier.",
        body2:
          "Please double-check your shipping address. Risk of loss transfers when the carrier takes possession, but contact us if a package is lost or damaged - we will help coordinate with the carrier where possible.",
        body3: "Customs, duties, or import fees (if applicable) are the customer's responsibility.",
      },
      returns: {
        title: "Returns & exchanges",
        eyebrow: "Policy TBD",
        body1: "We want you to love your cap. A finalized return/exchange policy is coming soon.",
        body2:
          "Suggested defaults (to be confirmed)- {returnsWindowDays}-day window from delivery, items must be unworn and in original condition, customer pays return shipping unless the item arrived damaged.",
        body3:
          "Until confirmed, please email {returnsEmail} with your order number and we'll handle requests case by case.",
      },
      product: {
        title: "Product details",
        body1:
          "Caps feature hand-applied, badge-style emblems. Minor variations or adhesive marks can occur because each piece is finished by hand.",
        body2: "Colors may look different across screens. Spot clean only- avoid high heat on the emblem.",
      },
      accounts: {
        title: "User accounts",
        body1:
          "If you create an account with NextAuth, keep your credentials secure and notify us of any unauthorized use.",
        body2:
          "You are responsible for all activity under your account. We may suspend or terminate accounts that violate these Terms or abuse the service.",
      },
      ip: {
        title: "Intellectual property",
        body1:
          "Lucky Caps, our logos, product imagery, and site content are owned by {businessName}. You may not use them without permission.",
      },
      prohibited: {
        title: "Prohibited use",
        bullets: {
          unlawful: "Using the site for unlawful, fraudulent, or harmful purposes.",
          interfere: "Interfering with or disrupting the site, including introducing malware or scraping without consent.",
          misuse: "Reselling, misrepresenting, or reverse-engineering the service or designs.",
          impersonation: "Impersonating others or providing false information.",
        },
      },
      disclaimers: {
        title: "Disclaimers & limitation of liability",
        body1:
          "The site and products are provided \"as is\" to the fullest extent permitted by law. We disclaim implied warranties of merchantability, fitness for a particular purpose, and non-infringement.",
        body2:
          "To the extent allowed by law, our total liability for any claim is limited to the amount you paid for the applicable order. Some regions do not allow certain limitations, so these may not apply to you.",
      },
      law: {
        title: "Governing law",
        body1:
          "These Terms are governed by the laws of {governingLaw}. Please replace this placeholder with your state/country before launch.",
      },
      updates: {
        title: "Updates to these terms",
        body1:
          "We may update these Terms from time to time. We will post the new date at the top. Continued use after updates means you accept the revised Terms.",
      },
      contact: {
        title: "Contact",
        body1: "Questions about these Terms- Email {supportEmail}.",
      },
    },
  },
  reviews: {
    title: "Customer Reviews",
    count: "({shown} / {total} reviews)",
    totalCount: "({count} reviews)",
    sortLabel: "Sort:",
    sortOptions: {
      recent: "Most recent",
      high: "Highest rating",
      low: "Lowest rating",
      helpful: "Most helpful",
    },
    filter: "Filter",
    filterByRating: "Filter by rating",
    starsLabel: "{count} star{suffix}",
    writeReview: "Write a review",
    loadingReviews: "Loading reviews...",
    ratingBreakdown: "Rating breakdown",
    noneYetTitle: "No reviews yet",
    noneYetCopy: "Be the first to review.",
    noneMatchTitle: "No reviews match your filters",
    noneMatchCopy: "Try adjusting your rating filters or clear filters to see all reviews.",
    showAll: "Show all reviews",
    modalTitle: "Write a review",
    forProduct: "for {product}",
    ratingLabel: "Rating",
    titleLabel: "Title",
    titlePlaceholder: "Premium quality",
    photoUrlLabel: "Photo URL (optional)",
    photoUrlPlaceholder: "https://...",
    reviewLabel: "Review",
    reviewPlaceholder: "Share fit, feel, and build quality.",
    variantLabel: "Variant (optional)",
    variantPlaceholder: "Snapback / Fitted / Trucker",
    sizeLabel: "Size (optional)",
    sizePlaceholder: "Adjustable / 7 1/4...",
    variantPlaceholderShort: "Snapback / Trucker",
    guidelinesAgree: "I agree to the community guidelines",
    submit: "Submit review",
    shareExperience: "Share your experience.",
    yourNameLabel: "Your name (optional)",
    namePlaceholder: "Alex Doe",
    rateStarsAria: "Rate {stars} star{suffix}",
    errors: {
      required: "Rating, title, and body are required.",
      requiredWithEmail: "Rating, title, body, and email are required.",
      guidelines: "Please agree to the community guidelines.",
      onlyPurchasers: "Only customers who purchased this item can leave a review.",
    },
    unableToLoadReviews: "Unable to load reviews.",
    thanks: "Thanks for your review!",
    unableToSubmitReview: "Unable to submit review.",
    anonymous: "Anonymous",
    helpful: "Helpful",
    verifiedPurchase: "Verified Purchase",
    variantValue: "Variant: {value}",
    sizeValue: "Size: {value}",
    photoCount: "{count} photo{suffix}",
    reported: "Reported",
    report: "Report",
  },
  admin: {
    title: "Admin",
    checkingAccess: "Checking access...",
    redirecting: "Redirecting...",
    products: "Products",
    orders: "Orders",
    promoCodes: "Promo codes",
    customLabels: "Custom Labels",
    logOut: "Log out",
    unauthorized: "Unauthorized. Please sign in as admin.",
    unauthorizedCheckAccess: "Unauthorized. Check admin access.",
    unableToLoadProducts: "Unable to load products.",
    unableToDuplicate: "Unable to duplicate.",
    unableToDelete: "Unable to delete.",
    unableToCreateProduct: "Unable to create product.",
    unableToLoadOrders: "Unable to load orders.",
    loadingOrder: "Loading order...",
    invalidOrderId: "Invalid order id.",
    unableToLoadOrder: "Unable to load order.",
    unableToLoadProduct: "Unable to load product.",
    unableToUpdateProduct: "Unable to update product.",
    unableToDeleteProduct: "Unable to delete product.",
    requestFailed: "Request failed (status {status}).",
    requiredField: "required",
    inventory: "Inventory",
    loadingProducts: "Loading products...",
    noProducts: "No products found.",
    product: "Product",
    category: "Category",
    price: "Price",
    stock: "Stock",
    flags: "Flags",
    inactive: "Inactive",
    edit: "Edit",
    editProduct: "Edit Product",
    duplicate: "Duplicate",
    addProduct: "Add Product",
    checkTranslations: "Check translations",
    backToProducts: "Back to Products",
    allStatuses: "All statuses",
    newestFirst: "Newest first",
    oldestFirst: "Oldest first",
    noOrdersYet: "No orders yet",
    ordersEmptyCopy: "Orders will appear here once customers checkout.",
    goToProducts: "Go to Products",
    loadingOrders: "Loading orders...",
    searchOrdersPlaceholder: "Search by order ID or email",
    orderTypeAccount: "Account",
    view: "View",
    manageOrder: "Manage Order",
    backToOrders: "Back to Orders",
    orderNotFound: "Order not found",
    savedChanges: "Saved changes",
    unableToSaveChanges: "Unable to save changes",
    itemsTitle: "Items",
    shippingTitle: "Shipping",
    customerTitle: "Customer",
    contactTitle: "Contact",
    totalsTitle: "Totals",
    trackingNotesTitle: "Tracking & Notes",
    shippingLabelTitle: "Shipping Labels",
    shippingPaidOnly: "Shipping labels are available after payment.",
    shippingPackageTemplate: "Package template",
    shippingCustomTemplate: "Custom package",
    shippingLength: "Length",
    shippingWidth: "Width",
    shippingHeight: "Height",
    shippingWeight: "Weight",
    shippingDistanceUnit: "Distance unit",
    shippingMassUnit: "Mass unit",
    shippingLabelFormat: "Label format",
    shippingLengthHelp: "Longest outside side of the package.",
    shippingWidthHelp: "Side-to-side outside measurement.",
    shippingHeightHelp: "Top-to-bottom outside measurement.",
    shippingWeightHelp: "Total packed weight using the selected weight unit.",
    shippingDistanceUnitHelp: "Applies to length, width, and height.",
    shippingMassUnitHelp: "Applies to the weight number.",
    shippingLabelFormatHelp: "PDF_4x6 is the standard printable label. ZPLII is for thermal label printers.",
    shippingDistanceUnit_in: "Inches (in)",
    shippingDistanceUnit_cm: "Centimeters (cm)",
    shippingMassUnit_oz: "Ounces (oz)",
    shippingMassUnit_lb: "Pounds (lb)",
    shippingMassUnit_g: "Grams (g)",
    shippingMassUnit_kg: "Kilograms (kg)",
    shippingLabelFormat_PDF_4x6: "PDF 4 x 6 label",
    shippingLabelFormat_ZPLII: "ZPLII thermal label",
    shippingGetRates: "Get Rates",
    shippingRatesTitle: "Rates",
    shippingNoRates: "No rates yet.",
    shippingBuyLabel: "Buy Label",
    shippingLabelReady: "Label ready",
    shippingDownloadLabel: "Download {format} label",
    shippingTrackingNumber: "Tracking number",
    shippingMarkShipped: "Mark as shipped",
    shippingRatesReady: "Rates loaded",
    shippingLabelPurchased: "Label purchased",
    shippingInvalidParcel: "Enter valid parcel details.",
    shippingRandomize: "Randomize",
    shippingSuggestedTemplate: "Suggested: {name} for {count} items.",
    shippingVolumeWarning: "Volume exceeds 1,728 in³ (USPS DIM threshold).",
    shippingLengthWarning: "Length ≥ 22 in may trigger oversize fees.",
    shippingLabelArchived: "Label archived.",
    shippingUnableToArchiveLabel: "Unable to archive label.",
    shippingLabelArchiving: "Archiving label...",
    shippingLabelNotArchived: "Label purchased but not archived yet.",
    shippingRetryArchive: "Retry archive",
    shippingPostage: "Postage",
    shippingEstimatedDays: "{days} days",
    shippingNoLabelPurchased: "No shipping label purchased yet.",
    shippingNoShipmentYet: "No shipment created yet.",
    manageTitle: "Manage",
    statusLabel: "Status",
    trackingNumberLabel: "Tracking Number",
    enterTrackingPlaceholder: "Enter tracking",
    trackingHelp: "Add tracking when shipped (optional).",
    adminNotesLabel: "Admin Notes",
    internalNotesPlaceholder: "Internal notes",
    markOrderConfirm: "Mark order as {status}-",
    markPaid: "Mark Paid",
    markShipped: "Mark Shipped",
    markDelivered: "Mark Delivered",
    cancelOrder: "Cancel Order",
    markRefunded: "Mark Refunded",
    unknownCustomer: "Unknown customer",
    noNotes: "No notes",
    deleteProductConfirm: "Delete this product-",
    deleteConfirm: "Delete {name}- This cannot be undone.",
    loadingProduct: "Loading product...",
    saveChanges: "Save Changes",
    productNotFound: "Product not found.",
    productNotFoundCopy: "The item may have been removed. Return to the dashboard to view the current catalog.",
    backToAdmin: "Back to Admin",
    orderConfirmationSent: "Order confirmation sent",
    shippingConfirmationSent: "Shipping confirmation sent",
    emailErrorLabel: "Email error: {message}",
    manualCreateTitle: "Create Manual Shipping Label",
    manualRecentTitle: "Recent Manual Labels",
    manualRecipientName: "Recipient name",
    manualRecipientEmail: "Recipient email",
    manualRecipientPhone: "Recipient phone",
    manualAddress1: "Address line 1",
    manualAddress2: "Address line 2 (optional)",
    manualCity: "City",
    manualState: "State / Province",
    manualZip: "ZIP / Postal code",
    manualCountry: "Country code (US, MX, etc.)",
    manualRecipientNamePlaceholder: "Full name on the label",
    manualRecipientEmailPlaceholder: "customer@example.com",
    manualRecipientPhonePlaceholder: "Phone number for carrier contact",
    manualAddress1Placeholder: "Street address and house number",
    manualAddress2Placeholder: "Apt, suite, unit, building",
    manualCityPlaceholder: "City",
    manualStatePlaceholder: "State or province code",
    manualZipPlaceholder: "ZIP or postal code",
    manualCountryPlaceholder: "US",
    manualRecipientNameHelp: "Required. This is the ship-to name printed on the label.",
    manualRecipientEmailHelp: "Optional, but useful for carrier notifications.",
    manualRecipientPhoneHelp: "Optional for some carriers, required by others.",
    manualAddress1Help: "Required. Enter the deliverable street address.",
    manualAddress2Help: "Use only for apartment, suite, unit, or building details.",
    manualStateHelp: "Use the state or province abbreviation when available, such as IA or CA.",
    manualCountryHelp: "Use the two-letter ISO country code Shippo expects, such as US or MX.",
    manualInvalidRecipient: "Enter the required recipient fields: name, address line 1, city, ZIP / postal code, and country.",
    manualRatesReady: "Rates loaded",
    manualLabelPurchased: "Manual label purchased",
    manualNoneYet: "No manual labels yet.",
    manualUnableToLoad: "Unable to load manual labels.",
    manualUnableToCreateDraft: "Unable to create manual shipment draft.",
    manualUnableToBuyLabel: "Unable to buy manual label.",
  },
  adminProductForm: {
    details: "Details",
    name: "Name",
    slug: "Slug",
    autoSlug: "Auto",
    slugHelp: "Must be unique; generated from the name.",
    description: "Description",
    descriptionPlaceholder: "Short product description",
    category: "Category",
    categoryPlaceholder: "Caps, beanies...",
    stock: "Stock",
    pricing: "Pricing",
    basePrice: "Base price",
    onSale: "On sale",
    salePricePlaceholder: "Sale price",
    originalPricePlaceholder: "Original price",
    saleHint:
      "When on sale, the red value is the sale price and the faint value is the original.",
    markNewDrop: "Mark as New Drop",
    sizesAvailable: "Available sizes",
    sizesHelp: "Choose available sizes. Leave empty to hide options.",
    images: "Images",
    addImageByUrl: "Add image by URL",
    photoUrlPlaceholder: "https://...",
    uploadForPreview: "Upload for preview",
    uploadHelp: "Select images to upload to Cloudinary. You can also paste URLs.",
    uploadFailed: "Upload failed",
    uploadFailedStatus: "Upload failed (status {status})",
    sessionExpired: "Your admin session expired. Please sign in again.",
    checkEnv:
      "Verify admin login and CLOUDINARY_* variables.",
    waitUploads: "Wait for uploads to finish.",
    invalidImages: "Images must be on Cloudinary or use http(s) URLs.",
    previewAlt: "Preview {index}",
    noImages: "No images yet. Add a URL or upload to Cloudinary.",
    imagePrimary: "Primary image",
    makePrimary: "Make primary",
    moveUp: "Move up",
    moveDown: "Move down",
  },
  adminPromoCodes: {
    noneYet: "No promo codes yet.",
    unableToLoad: "Unable to load promo codes.",
    unableToUpdate: "Unable to update promo code.",
    unableToCreate: "Unable to create promo code.",
    notFound: "Promo code not found.",
    statusActive: "Active",
    statusInactive: "Inactive",
    discountTypeLabel: "Discount type",
    discountTypePercent: "Percent",
    discountTypeAmount: "Amount",
    percentOff: "Percent off",
    amountOff: "Amount off",
    currency: "Currency",
    minSubtotal: "Min subtotal",
    maxRedemptions: "Max redemptions",
    redeemed: "Redeemed",
    window: "Window",
    startsAt: "Starts at",
    endsAt: "Ends at",
    windowRange: "{start} → {end}",
    codePlaceholder: "SAVE10",
    currencyPlaceholder: "usd",
    percentPlaceholder: "10",
    amountPlaceholder: "5.00",
    minSubtotalPlaceholder: "20.00",
    maxRedemptionsPlaceholder: "100",
  },
};

const es: Dictionary = {

  nav: {
    home: "Inicio",
    shop: "Tienda",
    story: "Historia",
    account: "Cuenta",
    admin: "Admin",
  },
  langToggle: {
    en: "EN",
    es: "ES",
  },
  brand: {
    name: "Lucky Caps",
  },
  common: {
    loading: "Cargando...",
    saving: "Guardando...",
    back: "Volver",
    backToShop: "Volver a la tienda",
    close: "Cerrar",
    save: "Guardar",
    cancel: "Cancelar",
    clear: "Limpiar",
    submitting: "Enviando...",
    add: "Agregar",
    remove: "Quitar",
    uploaded: "Subido",
    failed: "Falló",
    delete: "Eliminar",
    submit: "Enviar",
    view: "Ver",
    manage: "Administrar",
    search: "Buscar",
    status: "Estado",
    actions: "Acciones",
    code: "Código",
    value: "Valor",
    enable: "Habilitar",
    disable: "Deshabilitar",
    created: "Creado",
    paid: "Pagado",
    shipped: "Enviado",
    delivered: "Entregado",
    cancelled: "Cancelado",
    refunded: "Reembolsado",
    none: "Ninguno",
    customer: "Cliente",
    type: "Tipo",
    order: "Pedido",
    items: "Artículos",
    subtotal: "Subtotal",
    discount: "Descuento",
    total: "Total",
    notes: "Notas",
    tracking: "Seguimiento",
    upload: "Subir",
    uploading: "Subiendo...",
    loadMore: "Cargar más",
  },
  header: {
    logoAlt: "Logo de Lucky Caps",
    accountMenu: "Menú de cuenta",
    openCart: "Abrir carrito",
    openMenu: "Abrir menú",
    closeMenu: "Cerrar menú",
    menuTitle: "Menú",
    switchToEnglish: "Cambiar a inglés",
    switchToSpanish: "Cambiar a español",
  },
  footer: {
    tagline: "Lucky Supply",
    blurb:
      "Gorras estilo badge hechas a mano en Des Moines, con emblemas trabajados al detalle y actitud de calle. Solo drops limitados para quien viste su propia identidad.",
    shop: "Tienda",
    stayInLoop: "Mantente al tanto",
    noSpam: "Sin spam. Solo lanzamientos y detrás de cámaras.",
    subscribed: "Listo. Revisa tu bandeja pronto.",
    subscribeError: "No pudimos guardar tu registro. Intenta de nuevo.",
    emailError: "Ingresa un correo válido.",
    consentError: "Confirma el consentimiento de marketing.",
    company: "Empresa",
    legal: "Legal",
    backToTop: "Volver arriba",
    emailPlaceholder: "tu@ejemplo.com",
    join: "Unirme",
    consent: "Envíenme drops y promos",
    copyright: "Ac {year} Lucky Caps. Todos los derechos reservados.",
    facts: {
      emblems: "Emblemas aplicados a mano",
      smallBatch: "Lotes pequeños",
      local: "Hechas en Des Moines",
    },
    badges: {
      craftedByHand: "Hechas a mano",
      limitedDrops: "Drops de edición limitada",
      boldIdentity: "Identidad sin filtro",
    },
    shopLinks: {
      snapbacks: "Snapbacks",
      fitted: "Fitted",
      trucker: "Trucker",
      beanies: "Beanies",
      custom: "Custom",
      shopAll: "Ver todo",
    },
    companyLinks: {
      story: "Historia",
      contact: "Contacto",
    },
    legalLinks: {
      privacy: "Privacidad",
      terms: "Términos",
    },
  },
  categories: {
    snapbacks: "Snapbacks",
    fitted: "Ajustadas",
    trucker: "Trucker",
    beanies: "Beanies",
    custom: "Personalizadas",
    dad_hat: "Dad Hat",
    rope: "Rope",
    five_panel: "Cinco paneles",
    flat_bill: "Visera plana",
    caps: "Gorras",
    gorra: "Gorra",
    snapbacks_2pk: "Snapbacks paquete de 2",
  },
  home: {
    heroImageAlt: "Fondo con gorra y brillo verde",
    heroHeadline: "Lleva tu suerte.",
    heroSub:
      "Gorras con emblemas aplicados a mano, hechas en Des Moines. Logos tipo emblema, terminadas a mano - para los que se mueven diferente.",
    heroBadge: "Gorras tipo emblema",
    heroCtaPrimary: "Ver colección",
    heroCtaSecondary: "Conoce al fundador",
    marquee: {
      emblems: "Emblemas aplicados a mano",
      badgeStyle: "Logos tipo emblema",
      smallBatch: "Lotes pequeños",
      desMoines: "Hechas en Des Moines",
    },
    newDrops: "Gorras destacadas",
    freshHeading: "Emblemas terminados a mano en lotes pequeños",
    explore: "Explorar",
    emptyNewDropsTitle: "Nuevos productos pronto.",
    emptyNewDropsCopy: "Vuelve pronto o explora toda la tienda.",
    featuredCategories: "Categorías destacadas",
    builtForEveryDrop: "Diseñadas para cada drop",
    featuredCopy: "Selecciones listas para emblemas en cualquier estilo.",
    viewAll: "Ver todo",
    shopCategoryAria: "Comprar categoría: {category}",
    shopNow: "Comprar ahora",
    emptyCategories: "Aún no hay categorías disponibles.",
    midBand: {
      one: "Emblemas aplicados a mano",
      two: "Lotes pequeños",
      three: "Hechas en Des Moines",
    },
    diff: {
      kicker: "Qué nos hace diferentes",
      title: "Qué hace diferente a Lucky Caps",
      copy: "Gorras reales hechas por un barbero-maker. Emblemas tipo insignia, aplicados a mano.",
      one: {
        title: "Emblemas aplicados a mano",
        copy: "Logos tipo emblema aplicados con adhesivo especial para un look limpio y elevado.",
      },
      two: {
        title: "Lotes pequeños",
        copy: "Diseñadas y armadas en tiradas limitadas para que cada lanzamiento sea especial.",
      },
      three: {
        title: "Hecho localmente",
        copy: "Creado por un barbero-maker en Des Moines, Iowa.",
      },
    },
    founder: {
      kicker: "El fundador",
      title: "Creado por Jesús Pérez",
      copy:
        "Barbero en Des Moines que empezó a crear gorras en noviembre de 2024 - convirtiendo emblemas en piezas con actitud.",
      cta: "Leer la historia",
      imageAlt: "Retrato de Jesús Pérez con una Lucky Cap",
    },
    luckySocial: "Lucky Social",
    seenOn: "Visto en la comunidad",
    socialImageAlt: "Galería social de Lucky Caps",
  },
  shop: {
    title: "Tienda",
    heading: "Drops premium",
    kicker: "Comprar colección",
    description: "Filtra por categoría, precio y estado para encontrar tu próxima gorra.",
    itemsCount: "{count} artículos",
    reset: "Restablecer",
    refine: "Refinar",
    filters: "Filtros",
    sort: "Ordenar",
    categories: "Categorías",
    noCategories: "No hay categorías disponibles.",
    priceRange: "Rango de precio",
    highlights: "Destacados",
    newDropsFilter: "Nuevos drops",
    onSale: "En oferta",
    showing: "Mostrando {shown} de {total} productos",
    featured: "Destacado",
    newest: "Más nuevo",
    priceLowHigh: "Precio: menor a mayor",
    priceHighLow: "Precio: mayor a menor",
    upTo: "Hasta",
    productsSuffix: "productos",
    newDrop: "Nuevo lanzamiento",
    sale: "Oferta",
    loadingProducts: "Cargando productos...",
    loadFailed: "No se pudieron cargar los productos. Intenta de nuevo.",
    emptyTitle: "Ningún producto coincide con tus filtros.",
    emptyCopy: "Ajusta los filtros o restablece para ver todo.",
    clearFilters: "Limpiar filtros",
    retryLoad: "Reintentar",
  },
  product: {
    variant: "Variante",
    size: "Talla",
    features: "Características",
    fastShipping: "Envío rápido",
    fastShippingCopy: "Despacho en 2 días hábiles.",
    returns: "Devoluciones 30 días",
    secureCheckout: "Pago seguro",
    secureCheckoutCopy: "Pagos cifrados para tu tranquilidad.",
    easyReturns: "Devoluciones fáciles",
    easyReturnsCopy: "Devoluciones sin complicaciones por 30 días.",
    trust: {
      title: "Hecho para tu confianza",
      secureCheckout: "Pago seguro con Stripe",
      trackedShipping: "Envíos con seguimiento en cada pedido",
      support: "Soporte: {email}",
      returns: "Política de devoluciones disponible en checkout",
    },
    decreaseQuantityAria: "Disminuir cantidad",
    increaseQuantityAria: "Aumentar cantidad",
    addToCart: "Añadir al carrito",
    outOfStock: "Agotado",
    addToWishlistAria: "Añadir a favoritos",
    productNotFound: "Producto no encontrado.",
    unableToLoadProduct: "No se pudo cargar el producto.",
    tryOtherProducts: "Prueba a ver otros productos.",
    reportReviewAria: "Reportar reseña",
    galleryImageAria: "Ver imagen {index} de {name}",
  },
  cart: {
    title: "Tu carrito",
    empty: "Tu carrito está vacío.",
    emptyCopy: "Explora la tienda y añade tu primer drop.",
    emptyPageTitle: "Tu carrito está vacío",
    emptyPageCopy: "Inicia un drop y agrega gorras Lucky Caps a tu carrito.",
    browseCollection: "Ver colección",
    noImage: "Sin imagen",
    variantFallback: "Estándar",
    sizeFallback: "Talla única",
    viewCart: "Ver carrito",
    checkout: "Pagar",
    orderSummary: "Resumen del pedido",
    shipping: "Envío",
    tax: "Impuestos",
    calculatedAtCheckout: "Se calcula en el checkout",
    continueShopping: "Seguir comprando",
    removeItemAria: "Quitar artículo",
  },
  checkout: {
    title: "Pagar",
    goToAdmin: "Ir a Admin",
    heading: "Completa tu pedido",
    autofill: "Autocompletar datos de prueba",
    notice: "Flujo seguro - pagos no habilitados todavía.",
    loadingCheckout: "Cargando checkout...",
    unableToPlaceOrder: "No se pudo realizar el pedido.",
    quoteLoadFailed: "No se pudo calcular envio ni total. Intenta de nuevo.",
    retryQuote: "Reintentar totales",
    requiredFieldsError: "Completa los campos obligatorios y agrega artículos al carrito.",
    missingCheckoutUrl: "Falta la URL de checkout.",
    shippingAddressTitle: "Dirección de envío",
    emptyTitle: "Tu carrito está vacío.",
    emptyCopy: "Agrega productos antes de pagar.",
    canceled: "Checkout cancelado. Puedes intentar de nuevo o actualizar tus datos abajo.",
    redirectingTitle: "Redirigiendo al pago",
    redirectingCopy: "Te enviamos a Stripe para completar el pago de forma segura.",
    finalizingTitle: "Finalizando tu pedido",
    finalizingCopy: "Confirmando el pago y creando tu pedido.",
    unableToFinalize: "No se pudo finalizar el pedido.",
    retryFinalization: "Reintentar finalizacion",
    returnToCheckout: "Volver al checkout",
    orderIdLabel: "ID de pedido",
    processing: "Procesando...",
    viewOrder: "Ver pedido",
    customer: "Cliente",
    email: "Correo",
    phone: "Teléfono",
    phoneOptional: "Teléfono (opcional)",
    shipping: "Envío",
    contactTitle: "Contacto",
    firstName: "Nombre",
    lastName: "Apellido",
    address1: "Dirección (línea 1)",
    address2: "Dirección (línea 2, opcional)",
    city: "Ciudad",
    state: "Estado",
    zip: "Código postal",
    country: "Pais",
    deliveryMethodTitle: "Metodo de entrega",
    delivery: {
      flat: { name: "Envio tarifa unica", description: "Una sola tarifa para todos los pedidos", eta: "Llega en 3-6 dias habiles" },
    },
    shippingTbdSelect: "Por definir - selecciona envio",
    totalTbd: "Por definir",
    taxLabel: "Impuestos ({rate}%)",
    free: "Gratis",
    payment: "Pago",
    cardDetails: "Detalles de tarjeta",
    orderNotes: "Notas del pedido",
    orderNotesPlaceholder: "Notas del pedido / instrucciones de bordado",
    orderSummary: "Resumen del pedido",
    promoCode: "Código promocional",
    promoSoon: "Códigos próximamente.",
    coupon: "Código de descuento",
    apply: "Aplicar",
    couponPlaceholder: "",
    placeOrder: "Proceder al pago",
    placingOrder: "Realizando pedido...",
    redirectNotice: "Serás redirigido a Stripe para pagar de forma segura.",
    shippingRequiredInline: "Selecciona una opción de envío para continuar.",
    shippingRequiredError: "Selecciona una opción de envío.",
    shippingPendingError: "El envío está pendiente. Selecciona una opción de envío.",
    promoApplied: "Promo aplicada",
    promoApply: "Aplicar",
    promoRemove: "Quitar",
    promoError: "Error de código promocional",
    promoErrors: {
      missing: "Ingresa un código promocional.",
      notFound: "El código promocional no es válido.",
      inactive: "El código promocional está inactivo.",
      notStarted: "El código promocional aún no está activo.",
      expired: "El código promocional expiró.",
      minSubtotal: "Requiere un subtotal de ${min}.",
      maxRedemptions: "El código alcanzó su límite de usos.",
      notAvailable: "El código no está disponible.",
      invalid: "No se pudo aplicar el código.",
    },
    trust: {
      title: "Compra con confianza",
      stripe: "Pago seguro con Stripe",
      shipping: "Envíos con seguimiento en cada pedido",
      support: "Soporte: support@luckycapsshop.com",
      returns: "Política de devoluciones disponible en checkout",
    },
  },
  order: {
    kicker: "Pedido",
    title: "Pedido {id}",
    statusValue: "Estado: {status}",
    paymentReceived: "Pago recibido. Estamos procesando tu pedido.",
    sizeValue: "Talla: {size}",
    qtyValue: "Cant.: {qty}",
    deliveryValue: "Entrega: {delivery}",
    na: "N/D",
    totalsTitle: "Totales",
  },
  auth: {
    signIn: "Iniciar sesión",
    signUp: "Crear cuenta",
    signOut: "Cerrar sesión",
    forgotPassword: "Restablecer contraseña",
    signInSubtitle: "Accede a tu cuenta de Lucky Caps.",
    signUpSubtitle:
      "Guarda tus drops, rastrea pedidos y adelántate a los lanzamientos.",
    forgotSubtitle:
      "Te enviaremos un enlace si existe una cuenta para este correo.",
    emailLabel: "Correo",
    passwordLabel: "Contraseña",
    firstNameLabel: "Nombre",
    lastNameLabel: "Apellido",
    password: "Contraseña",
    confirmPassword: "Confirmar contraseña",
    confirmPasswordLabel: "Confirmar contraseña",
    emailPlaceholder: "tu@ejemplo.com",
    rememberMe: "Recordarme",
    forgotPasswordLink: "¿Olvidaste tu contraseña-",
    continueAsGuest: "Continuar como invitado",
    googleSoon: "Iniciar con Google (próximamente)",
    signingIn: "Iniciando sesión...",
    signedIn: "Sesión iniciada. Redirigiendo...",
    noAccount: "¿No tienes cuenta-",
    createOne: "Crea una",
    alreadyAccount: "¿Ya tienes cuenta-",
    minPasswordHint: "Mínimo 8 caracteres.",
    termsAgree: "Acepto los Términos y Privacidad",
    marketingOptIn: "Envíenme drops y promos",
    unsubscribeCopy: "Puedes darte de baja cuando quieras.",
    reset: {
      emailSent: "Correo enviado",
      sentCopy: "Si existe una cuenta para {email}, recibirás un enlace en breve.",
      returnToSignIn: "Volver a iniciar sesión",
      sendLink: "Enviar enlace",
      remembered: "¿Recordaste tu contraseña-",
    },
    errors: {
      invalidEmail: "Ingresa un correo valido.",
      nameRequired: "El nombre es obligatorio.",
      passwordRequired: "La contrasena es obligatoria.",
      passwordTooShort: "La contrasena debe tener al menos 8 caracteres.",
      passwordsMustMatch: "Las contrasenas deben coincidir.",
      mustAgreeTerms: "Debes aceptar los Terminos y Privacidad.",
      invalidCredentials: "Correo o contrasena invalidos.",
      signInFailed: "No pudimos iniciar sesion. Intenta de nuevo.",
      unableToCreateAccount: "No se pudo crear la cuenta.",
    },
    success: {
      signedIn: "Listo. Redirigiendo...",
    },
},
  account: {
    title: "Tu hub Lucky",
    heading: "Cuenta",
    guest: "Invitado",
    loadingAccount: "Cargando cuenta...",
    welcome: "Bienvenido, {name}",
    intro: "Administra tu perfil, pedidos y preferencias.",
    backToShop: "Volver a la tienda",
    profile: "Perfil",
    orders: "Historial de pedidos",
    addresses: "Direcciones",
    settings: "Ajustes",
    name: "Nombre",
    email: "Correo",
    loadingOrders: "Cargando pedidos...",
    noOrders: "Aún no hay pedidos",
    noOrdersCopy: "Inicia un drop para ver tu historial aquí.",
    shopCollection: "Comprar colección",
    noAddresses: "No hay direcciones guardadas",
    orderNumber: "Pedido #{id}",
    statusValue: "Estado: {status}",
    totalValue: "Total ${total}",
    qty: "Cant. {qty}",
    noAddressesCopy: "Agrega una dirección para acelerar el pago.",
    addAddressSoon: "Agregar dirección (próximamente)",
    newPassword: "Nueva contraseña",
    confirmPasswordLabel: "Confirmar contraseña",
    updatePasswordSoon: "Actualizar contraseña (próximamente)",
    signOutTitle: "Cerrar sesión",
    signOutCopy: "Cierra sesión y vuelve al inicio.",
    unableToLoadOrders: "No se pudieron cargar los pedidos.",
  },
  about: {
    badge: "La historia",
    title: "La historia",
    heroHeading: "Conoce a Jesús Pérez",
    heroSub:
      "Fundador de Lucky Caps. Barbero en Des Moines. Comenzó en noviembre de 2024 con una idea simple: convertir emblemas tipo insignia en gorras con presencia.",
    imageAlt: "Foto de Jesús Pérez",
    fact: {
      role: "Fundador y CEO, Lucky Caps",
      location: "Barbero en Des Moines, Iowa",
      started: "Inició en noviembre de 2024",
    },
    sections: {
      kicker: "Notas del fundador",
      how: {
        title: "Cómo empezó",
        copy:
          "En noviembre de 2024, Jesús empezó a crear y vender gorras localmente en Des Moines - armando cada pieza a mano y aprendiendo qué busca la gente para su estilo diario.",
      },
      what: {
        title: "Qué hace diferente a Lucky Caps",
        copy:
          "Cada gorra lleva un emblema tipo insignia aplicado con adhesivo especial para un acabado limpio y elevado. Hechas en lotes pequeños y revisadas antes de enviarse.",
      },
      why: {
        title: "Por qué \"Lleva tu suerte\"",
        copy:
          "Lucky Caps se trata de creer en ti - mantenerte constante y dejar que tu estilo hable.",
      },
    },
    whyTitle: "Lleva tu suerte",
    whyHeadline: "Por qué Lucky Caps",
    whyCopy:
      "Hechas para quienes se mueven diferente. Se sienten como una insignia que puedes usar en cualquier parte.",
    ctaShop: "Ver la colección",
  },
  contact: {
    title: "Contacto",
    copy: "Necesitas ayuda- Escribenos a support@luckycapsshop.com.",
  },
  legal: {
    lastUpdated: "Ultima actualizacion {date}",
    toc: "En esta pagina",
    disclaimer:
      "Este contenido es informativo y no es asesoria legal. Por favor revisalo con tu abogado.",
    policyTbd: "Politica pendiente",
    todo: "TODO: confirmar con el equipo antes del lanzamiento.",
    contact: "Escribenos a {email}",
  },
  privacy: {
    title: "Politica de privacidad",
    description:
      "Como Lucky Caps recopila, usa y comparte datos para operar la tienda, enviar pedidos y comunicarte.",
    sections: {
      overview: {
        title: "Resumen",
        body1:
          "Esta politica explica como {businessName} (\"nosotros\") maneja la informacion cuando navegas, creas una cuenta o compras en luckycapsshop.com.",
        body2:
          "Usamos Stripe para pagos, Shippo para guias y seguimiento, Resend para correos transaccionales, Neon Postgres para la base de datos y NextAuth para sesiones de cuenta.",
      },
      infoWeCollect: {
        title: "Informacion que recopilamos",
        items: {
          account: "Datos de cuenta si inicias sesion (correo y nombre que ingresas via NextAuth).",
          checkout:
            "Datos de pedido y checkout- articulos, direccion de envio, correo, telefono (si lo compartes) y opciones de entrega.",
          payment: "Confirmaciones de pago desde Stripe (no guardamos los numeros completos de tarjeta).",
          device:
            "Datos de dispositivo y registro como IP, tipo de navegador y tiempos de interaccion para seguridad y prevencion de fraude.",
          media: "Fotos que subes para trabajos personalizados o reseñas (almacenadas en Cloudinary).",
        },
      },
      howWeUse: {
        title: "Como usamos la informacion",
        bullets: {
          fulfill: "Preparar y enviar pedidos, incluyendo crear etiquetas con Shippo.",
          communicate:
            "Enviar confirmaciones, actualizaciones de envio y avisos de cuenta via Resend desde {replyToEmail}.",
          support: "Dar soporte y responder preguntas enviadas a {supportEmail}.",
          auth: "Autenticar cuentas y mantener sesiones activas con NextAuth.",
          improve: "Mantener y mejorar el sitio, prevenir fraude/abuso y proteger el servicio.",
          marketing: "Enviar correos de marketing solo si aceptas \"Enterate de las novedades\"; puedes darte de baja cuando quieras.",
        },
      },
      marketing: {
        title: "Opciones de marketing",
        body1:
          "El registro a correos de marketing es opcional y separado de los correos transaccionales. Cada correo de marketing incluye enlace para darte de baja.",
        body2:
          "Tambien puedes escribir a {supportEmail} para salir de la lista. Los correos transaccionales de pedidos siguen llegando cuando son necesarios.",
      },
      cookies: {
        title: "Cookies y analitica",
        body1: "Usamos cookies esenciales para mantener tu sesion, carrito y preferencia de idioma.",
        body2:
          "Las herramientas de analitica/ads aun no estan definidas. TODO- confirmar y listar cualquier proveedor antes del lanzamiento.",
        body3:
          "Puedes gestionar cookies en la configuracion de tu navegador; desactivar algunas puede afectar el login o el checkout.",
      },
      sharing: {
        title: "Como compartimos informacion",
        intro: "Compartimos datos solo con proveedores necesarios para operar Lucky Caps:",
        items: {
          stripe: "Stripe para procesar pagos y prevenir fraude.",
          shippo: "Shippo para tarifas, etiquetas y seguimiento.",
          resend: "Resend para enviar correos.",
          database: "Neon Postgres (via Netlify) para guardar pedidos, cuentas y productos.",
          cloudinary: "Cloudinary para hospedar imagenes de productos y de usuarios.",
          hosting: "Infraestructura Netlify/Next.js para alojar la tienda.",
        },
        closing:
          "No vendemos informacion personal. Si necesitamos compartir datos por obligacion legal o para proteger el servicio, lo haremos conforme a la ley.",
      },
      retention: {
        title: "Conservacion de datos",
        body1:
          "Mantenemos registros de pedidos y cuentas mientras sea necesario para operar la tienda, cumplir la ley, resolver disputas y conservar registros fiscales/financieros.",
        body2:
          "Si solicitas eliminacion, borraremos o anonimizaremos lo que no debamos conservar. Algunos datos pueden permanecer en respaldos por un tiempo limitado.",
      },
      security: {
        title: "Seguridad",
        body1:
          "Usamos medidas razonables (controles de acceso, cifrado en transito, roles de admin) para proteger los datos en Neon Postgres y en nuestros proveedores.",
        body2:
          "Ningun metodo de transmision o almacenamiento es 100% seguro. Si ocurre un problema que te afecte, te contactaremos con los datos que nos diste.",
      },
      children: {
        title: "Privacidad de menores",
        body:
          "Lucky Caps no esta dirigida a menores de 13. Si crees que un menor compartio informacion, escribe a {supportEmail} y la borraremos donde aplique.",
      },
      rights: {
        title: "Tus derechos y opciones",
        body1: "Puedes pedir acceso, correccion o eliminacion de tu informacion escribiendo a {supportEmail}.",
        body2:
          "Si estas en una region con derechos especificos (por ejemplo UE/Reino Unido/California), mencionarlo nos ayuda a seguir el proceso correcto. TODO- agregar lenguaje especifico cuando se confirme el alcance.",
      },
      updates: {
        title: "Actualizaciones de esta politica",
        body:
          "Podemos actualizar esta politica y cambiaremos la fecha de \"Ultima actualizacion\" cuando suceda. Si el cambio es importante, avisaremos brevemente en el sitio.",
      },
      contact: {
        title: "Contactanos",
        body: "Preguntas sobre privacidad- Escribe a {supportEmail}.",
      },
    },
  },
  terms: {
    title: "Terminos del servicio",
    description: "Las reglas para usar Lucky Caps y comprar nuestras gorras de edicion limitada.",
    sections: {
      overview: {
        title: "Resumen / Aceptacion de terminos",
        body1: "Al usar o comprar en Lucky Caps aceptas estos Terminos y la Politica de privacidad.",
        body2:
          "Si no estas de acuerdo, no uses el sitio. Podemos actualizar los Terminos; si sigues usando el sitio despues del cambio, aceptas la version actualizada.",
      },
      eligibility: {
        title: "Elegibilidad",
        body1:
          "Debes tener al menos 18 anos (o ser mayor de edad en tu region) o contar con consentimiento de un tutor para comprar o crear cuenta.",
        body2:
          "Debes proporcionar informacion correcta. Podemos rechazar o cancelar pedidos a discrecion (por ejemplo por sospecha de fraude o falta de inventario).",
      },
      orders: {
        title: "Pedidos y pago",
        bullets: {
          payment: "Los pagos se procesan con Stripe. No almacenamos los numeros completos de tarjeta.",
          pricing: "Los precios y disponibilidad pueden cambiar. Impuestos y envio se muestran en el checkout.",
          auth:
            "Al enviar un pedido autorizas que cobremos el total (impuestos, envio y descuentos aplicados) con tu metodo de pago a traves de Stripe.",
          promos:
            "Promos o cupones pueden tener limite de tiempo, uso o inventario; no se combinan salvo que se indique.",
          confirmation:
            "El pedido queda confirmado cuando lo aceptamos y se procesa el cobro. Si no podemos cumplirlo, lo cancelaremos y te reembolsaremos.",
        },
      },
      shipping: {
        title: "Envio y entrega",
        body1:
          "Enviamos con etiquetas generadas por Shippo. Las opciones y tiempos estimados se muestran en el checkout; la entrega real depende del transportista.",
        body2:
          "Revisa tu direccion de envio. El riesgo pasa al transportista al aceptar el paquete, pero si se pierde o daña, avisanos y ayudaremos a coordinar con la paqueteria cuando sea posible.",
        body3: "Cualquier impuesto o arancel de importacion (si aplica) es responsabilidad del cliente.",
      },
      returns: {
        title: "Devoluciones y cambios",
        eyebrow: "Politica pendiente",
        body1: "Queremos que te guste tu gorra. La politica final de devoluciones/cambios estara pronto.",
        body2:
          "Sugerencia a confirmar- ventana de {returnsWindowDays} dias desde la entrega, articulos sin usar y en buen estado, el cliente paga el envio de regreso salvo que llegue dañado.",
        body3:
          "Mientras tanto, escribe a {returnsEmail} con tu numero de pedido y veremos cada caso de forma individual.",
      },
      product: {
        title: "Detalles de producto",
        body1:
          "Las gorras llevan emblemas tipo insignia aplicados a mano. Puede haber leves variaciones o marcas porque cada pieza se termina manualmente.",
        body2: "Los colores pueden variar segun la pantalla. Limpia con paño humedo; evita calor alto sobre el emblema.",
      },
      accounts: {
        title: "Cuentas de usuario",
        body1:
          "Si creas una cuenta con NextAuth, manten seguras tus credenciales y avisanos ante uso no autorizado.",
        body2:
          "Eres responsable de toda actividad en tu cuenta. Podemos suspender o cerrar cuentas que violen estos Terminos o abusen del servicio.",
      },
      ip: {
        title: "Propiedad intelectual",
        body1:
          "Lucky Caps, nuestros logos, imagenes de productos y contenido del sitio pertenecen a {businessName}. No puedes usarlos sin permiso.",
      },
      prohibited: {
        title: "Usos prohibidos",
        bullets: {
          unlawful: "Usar el sitio para fines ilegales, fraudulentos o dañinos.",
          interfere: "Interferir o interrumpir el sitio, incluyendo malware o scraping sin permiso.",
          misuse: "Revender, suplantar o hacer ingenieria inversa del servicio o diseños.",
          impersonation: "Hacerse pasar por otros o dar informacion falsa.",
        },
      },
      disclaimers: {
        title: "Avisos y limitacion de responsabilidad",
        body1:
          "El sitio y los productos se ofrecen \"tal cual\" en la medida que permita la ley. Rechazamos garantias implicitas de comerciabilidad, adecuacion e inafectacion.",
        body2:
          "Cuando la ley lo permita, nuestra responsabilidad total por cualquier reclamo se limita al monto que pagaste por ese pedido. Algunas regiones no permiten estas limitaciones.",
      },
      law: {
        title: "Ley aplicable",
        body1:
          "Estos Terminos se rigen por las leyes de {governingLaw}. Reemplaza este marcador con tu estado/pais antes del lanzamiento.",
      },
      updates: {
        title: "Actualizaciones de estos terminos",
        body1:
          "Podemos actualizar estos Terminos. Publicaremos la nueva fecha arriba. Si sigues usando el sitio despues, aceptas la version actualizada.",
      },
      contact: {
        title: "Contacto",
        body1: "Preguntas sobre estos Terminos- Escribe a {supportEmail}.",
      },
    },
  },
  reviews: {
    title: "Reseñas de clientes",
    count: "({shown} / {total} reseñas)",
    totalCount: "({count} reseñas)",
    sortLabel: "Ordenar:",
    sortOptions: {
      recent: "Más recientes",
      high: "Mejor puntuación",
      low: "Peor puntuación",
      helpful: "Más útiles",
    },
    filter: "Filtrar",
    filterByRating: "Filtrar por puntuación",
    starsLabel: "{count} estrella{suffix}",
    writeReview: "Escribir reseña",
    loadingReviews: "Cargando reseñas...",
    ratingBreakdown: "Desglose de puntuación",
    noneYetTitle: "Aún no hay reseñas",
    noneYetCopy: "Sé el primero en opinar.",
    noneMatchTitle: "No hay reseñas que coincidan con tus filtros",
    noneMatchCopy: "Prueba ajustar los filtros o limpiarlos para ver todas las reseñas.",
    showAll: "Ver todas las reseñas",
    modalTitle: "Escribir reseña",
    forProduct: "para {product}",
    ratingLabel: "Puntuación",
    titleLabel: "Título",
    titlePlaceholder: "Calidad premium",
    photoUrlLabel: "URL de foto (opcional)",
    photoUrlPlaceholder: "https://...",
    reviewLabel: "Reseña",
    reviewPlaceholder: "Cuenta el ajuste, la sensación y la calidad.",
    variantLabel: "Variante (opcional)",
    variantPlaceholder: "Snapback / Fitted / Trucker",
    sizeLabel: "Talla (opcional)",
    sizePlaceholder: "Ajustable / 7 1/4...",
    variantPlaceholderShort: "Snapback / Trucker",
    guidelinesAgree: "Acepto las normas de la comunidad",
    submit: "Enviar reseña",
    shareExperience: "Comparte tu experiencia.",
    yourNameLabel: "Tu nombre (opcional)",
    namePlaceholder: "Alex Doe",
    rateStarsAria: "Calificar con {stars} estrella{suffix}",
    errors: {
      required: "La puntuación, el título y la reseña son obligatorios.",
      requiredWithEmail: "La puntuación, el título, la reseña y el correo son obligatorios.",
      guidelines: "Acepta las normas de la comunidad.",
      onlyPurchasers: "Solo clientes que compraron este artículo pueden dejar una reseña.",
    },
    unableToLoadReviews: "No se pudieron cargar las reseñas.",
    thanks: "¡Gracias por tu reseña!",
    unableToSubmitReview: "No se pudo enviar la reseña.",
    anonymous: "Anónimo",
    helpful: "Útil",
    verifiedPurchase: "Compra verificada",
    variantValue: "Variante: {value}",
    sizeValue: "Talla: {value}",
    photoCount: "{count} foto{suffix}",
    reported: "Reportada",
    report: "Reportar",
  },
  admin: {
    title: "Admin",
    checkingAccess: "Verificando acceso...",
    redirecting: "Redirigiendo...",
    products: "Productos",
    orders: "Pedidos",
    promoCodes: "Códigos promocionales",
    customLabels: "Etiquetas manuales",
    logOut: "Cerrar sesión",
    unauthorized: "No autorizado. Inicia sesión como admin.",
    unauthorizedCheckAccess: "No autorizado. Verifica el acceso de admin.",
    unableToLoadProducts: "No se pudieron cargar los productos.",
    unableToDuplicate: "No se pudo duplicar.",
    unableToDelete: "No se pudo eliminar.",
    unableToCreateProduct: "No se pudo crear el producto.",
    unableToLoadOrders: "No se pudieron cargar los pedidos.",
    loadingOrder: "Cargando pedido...",
    invalidOrderId: "ID de pedido inválido.",
    unableToLoadOrder: "No se pudo cargar el pedido.",
    unableToLoadProduct: "No se pudo cargar el producto.",
    unableToUpdateProduct: "No se pudo actualizar el producto.",
    unableToDeleteProduct: "No se pudo eliminar el producto.",
    requestFailed: "La solicitud falló (estado {status}).",
    requiredField: "obligatorio",
    inventory: "Inventario",
    loadingProducts: "Cargando productos...",
    noProducts: "No se encontraron productos.",
    product: "Producto",
    category: "Categoría",
    price: "Precio",
    stock: "Stock",
    flags: "Indicadores",
    inactive: "Inactivo",
    edit: "Editar",
    editProduct: "Editar producto",
    duplicate: "Duplicar",
    addProduct: "Agregar producto",
    checkTranslations: "Revisar traducciones",
    backToProducts: "Volver a productos",
    allStatuses: "Todos los estados",
    newestFirst: "Más nuevos primero",
    oldestFirst: "Más antiguos primero",
    noOrdersYet: "Aún no hay pedidos",
    ordersEmptyCopy: "Los pedidos aparecerán aquí cuando los clientes paguen.",
    goToProducts: "Ir a productos",
    loadingOrders: "Cargando pedidos...",
    searchOrdersPlaceholder: "Buscar por ID de pedido o correo",
    orderTypeAccount: "Cuenta",
    view: "Ver",
    manageOrder: "Administrar pedido",
    backToOrders: "Volver a pedidos",
    orderNotFound: "Pedido no encontrado",
    savedChanges: "Cambios guardados",
    unableToSaveChanges: "No se pudieron guardar los cambios",
    itemsTitle: "Artículos",
    shippingTitle: "Envío",
    customerTitle: "Cliente",
    contactTitle: "Contacto",
    totalsTitle: "Totales",
    trackingNotesTitle: "Seguimiento y notas",
    shippingLabelTitle: "Etiquetas de envío",
    shippingPaidOnly: "Las etiquetas están disponibles después del pago.",
    shippingPackageTemplate: "Plantilla de paquete",
    shippingCustomTemplate: "Paquete personalizado",
    shippingLength: "Largo",
    shippingWidth: "Ancho",
    shippingHeight: "Alto",
    shippingWeight: "Peso",
    shippingDistanceUnit: "Unidad de distancia",
    shippingMassUnit: "Unidad de masa",
    shippingLabelFormat: "Formato de etiqueta",
    shippingLengthHelp: "El lado exterior mas largo del paquete.",
    shippingWidthHelp: "Medida exterior de lado a lado.",
    shippingHeightHelp: "Medida exterior de arriba hacia abajo.",
    shippingWeightHelp: "Peso total del paquete usando la unidad de peso seleccionada.",
    shippingDistanceUnitHelp: "Aplica a largo, ancho y alto.",
    shippingMassUnitHelp: "Aplica al numero de peso.",
    shippingLabelFormatHelp: "PDF_4x6 es la etiqueta imprimible estandar. ZPLII es para impresoras termicas.",
    shippingDistanceUnit_in: "Pulgadas (in)",
    shippingDistanceUnit_cm: "Centimetros (cm)",
    shippingMassUnit_oz: "Onzas (oz)",
    shippingMassUnit_lb: "Libras (lb)",
    shippingMassUnit_g: "Gramos (g)",
    shippingMassUnit_kg: "Kilogramos (kg)",
    shippingLabelFormat_PDF_4x6: "Etiqueta PDF 4 x 6",
    shippingLabelFormat_ZPLII: "Etiqueta termica ZPLII",
    shippingGetRates: "Obtener tarifas",
    shippingRatesTitle: "Tarifas",
    shippingNoRates: "Sin tarifas todavía.",
    shippingBuyLabel: "Comprar etiqueta",
    shippingLabelReady: "Etiqueta lista",
    shippingDownloadLabel: "Descargar etiqueta {format}",
    shippingTrackingNumber: "Número de seguimiento",
    shippingMarkShipped: "Marcar como enviado",
    shippingRatesReady: "Tarifas cargadas",
    shippingLabelPurchased: "Etiqueta comprada",
    shippingInvalidParcel: "Ingresa detalles válidos del paquete.",
    shippingRandomize: "Aleatorio",
    shippingSuggestedTemplate: "Sugerido: {name} para {count} artículos.",
    shippingVolumeWarning: "El volumen supera 1.728 in³ (límite DIM de USPS).",
    shippingLengthWarning: "El largo ≥ 22 in puede generar recargos por sobredimensión.",
    shippingLabelArchived: "Etiqueta archivada.",
    shippingUnableToArchiveLabel: "No se pudo archivar la etiqueta.",
    shippingLabelArchiving: "Archivando etiqueta...",
    shippingLabelNotArchived: "Etiqueta comprada, pero aún no archivada.",
    shippingRetryArchive: "Reintentar archivado",
    shippingPostage: "Franqueo",
    shippingEstimatedDays: "{days} días",
    shippingNoLabelPurchased: "Aún no se ha comprado una etiqueta de envío.",
    shippingNoShipmentYet: "Aún no se ha creado un envío.",
    manageTitle: "Administrar",
    statusLabel: "Estado",
    trackingNumberLabel: "Número de seguimiento",
    enterTrackingPlaceholder: "Ingresar seguimiento",
    trackingHelp: "Agrega el número de seguimiento cuando se envíe (opcional).",
    adminNotesLabel: "Notas internas",
    internalNotesPlaceholder: "Notas internas",
    markOrderConfirm: "¿Marcar pedido como {status}?",
    markPaid: "Marcar como pagado",
    markShipped: "Marcar como enviado",
    markDelivered: "Marcar como entregado",
    cancelOrder: "Cancelar pedido",
    markRefunded: "Marcar como reembolsado",
    unknownCustomer: "Cliente desconocido",
    noNotes: "Sin notas",
    deleteProductConfirm: "¿Eliminar este producto?",
    deleteConfirm: "¿Eliminar {name}? Esto no se puede deshacer.",
    loadingProduct: "Cargando producto...",
    saveChanges: "Guardar cambios",
    productNotFound: "Producto no encontrado.",
    productNotFoundCopy: "Es posible que se haya eliminado. Vuelve al panel para ver el catálogo actual.",
    backToAdmin: "Volver a Admin",
    orderConfirmationSent: "Confirmación de pedido enviada",
    shippingConfirmationSent: "Confirmación de envío enviada",
    emailErrorLabel: "Error de correo: {message}",
    manualCreateTitle: "Crear etiqueta manual de envío",
    manualRecentTitle: "Etiquetas manuales recientes",
    manualRecipientName: "Nombre del destinatario",
    manualRecipientEmail: "Correo del destinatario",
    manualRecipientPhone: "Teléfono del destinatario",
    manualAddress1: "Dirección línea 1",
    manualAddress2: "Dirección línea 2 (opcional)",
    manualCity: "Ciudad",
    manualState: "Estado / Provincia",
    manualZip: "Código postal",
    manualCountry: "Código de país (US, MX, etc.)",
    manualRecipientNamePlaceholder: "Nombre completo en la etiqueta",
    manualRecipientEmailPlaceholder: "cliente@ejemplo.com",
    manualRecipientPhonePlaceholder: "Telefono para contacto del transportista",
    manualAddress1Placeholder: "Calle y numero de casa",
    manualAddress2Placeholder: "Apto, suite, unidad, edificio",
    manualCityPlaceholder: "Ciudad",
    manualStatePlaceholder: "Codigo de estado o provincia",
    manualZipPlaceholder: "Codigo postal",
    manualCountryPlaceholder: "US",
    manualRecipientNameHelp: "Obligatorio. Es el nombre de destino impreso en la etiqueta.",
    manualRecipientEmailHelp: "Opcional, pero util para notificaciones del transportista.",
    manualRecipientPhoneHelp: "Opcional para algunos transportistas, obligatorio para otros.",
    manualAddress1Help: "Obligatorio. Ingresa la direccion de entrega.",
    manualAddress2Help: "Usa esto solo para apartamento, suite, unidad o edificio.",
    manualStateHelp: "Usa la abreviatura del estado o provincia si existe, como IA o CA.",
    manualCountryHelp: "Usa el codigo ISO de dos letras que Shippo espera, como US o MX.",
    manualInvalidRecipient: "Ingresa los campos obligatorios del destinatario: nombre, direccion linea 1, ciudad, codigo postal y pais.",
    manualRatesReady: "Tarifas cargadas",
    manualLabelPurchased: "Etiqueta manual comprada",
    manualNoneYet: "Aún no hay etiquetas manuales.",
    manualUnableToLoad: "No se pudieron cargar las etiquetas manuales.",
    manualUnableToCreateDraft: "No se pudo crear el borrador de envío manual.",
    manualUnableToBuyLabel: "No se pudo comprar la etiqueta manual.",
  },
  adminProductForm: {
    details: "Detalles",
    name: "Nombre",
    slug: "Slug",
    autoSlug: "Auto",
    slugHelp: "Debe ser único; se genera desde el nombre.",
    description: "Descripción",
    descriptionPlaceholder: "Descripción corta del producto",
    category: "Categoría",
    categoryPlaceholder: "Gorras, beanies...",
    stock: "Stock",
    pricing: "Precios",
    basePrice: "Precio base",
    onSale: "En oferta",
    salePricePlaceholder: "Precio oferta",
    originalPricePlaceholder: "Precio original",
    saleHint:
      "En oferta, el precio rojo es el de oferta y el valor tenue es el original tachado.",
    markNewDrop: "Marcar como Nuevo Drop",
    sizesAvailable: "Tallas disponibles",
    sizesHelp: "Elige las tallas disponibles. Déjalo vacío para ocultar opciones.",
    images: "Imágenes",
    addImageByUrl: "Agregar imagen por URL",
    photoUrlPlaceholder: "https://...",
    uploadForPreview: "Subir para vista previa",
    uploadHelp: "Selecciona imágenes para subir a Cloudinary. También puedes pegar URLs.",
    uploadFailed: "Falló la subida",
    uploadFailedStatus: "Falló la subida (estado {status})",
    sessionExpired: "Tu sesión de admin expiró. Inicia sesión de nuevo.",
    checkEnv:
      "Verifica el login de admin y las variables CLOUDINARY_*.",
    waitUploads: "Espera a que terminen las subidas.",
    invalidImages: "Las imágenes deben estar en Cloudinary o usar URLs http(s).",
    previewAlt: "Vista previa {index}",
    noImages: "Aún no hay imágenes. Agrega una URL o sube a Cloudinary.",
    imagePrimary: "Imagen principal",
    makePrimary: "Hacer principal",
    moveUp: "Subir",
    moveDown: "Bajar",
  },
  adminPromoCodes: {
    noneYet: "Aún no hay códigos promocionales.",
    unableToLoad: "No se pudieron cargar los códigos promocionales.",
    unableToUpdate: "No se pudo actualizar el código promocional.",
    unableToCreate: "No se pudo crear el código promocional.",
    notFound: "Código promocional no encontrado.",
    statusActive: "Activo",
    statusInactive: "Inactivo",
    discountTypeLabel: "Tipo de descuento",
    discountTypePercent: "Porcentaje",
    discountTypeAmount: "Monto",
    percentOff: "Porcentaje",
    amountOff: "Monto",
    currency: "Moneda",
    minSubtotal: "Subtotal mínimo",
    maxRedemptions: "Máx. usos",
    redeemed: "Canjeado",
    window: "Vigencia",
    startsAt: "Empieza",
    endsAt: "Termina",
    windowRange: "{start} → {end}",
    codePlaceholder: "GUARDAR10",
    currencyPlaceholder: "usd",
    percentPlaceholder: "10",
    amountPlaceholder: "5.00",
    minSubtotalPlaceholder: "20.00",
    maxRedemptionsPlaceholder: "100",
  },
};

export const dictionaries: Record<Language, Dictionary> = {
  EN: en,
  ES: es,
};

const MISSING_TRANSLATION_PREFIX = "MISSING_TRANSLATION:";

function walkDictionary(
  node: Dictionary,
  path: string[] = [],
  visit?: (path: string, value: string) => void,
) {
  Object.entries(node).forEach(([key, value]) => {
    const nextPath = [...path, key];
    if (typeof value === "string") {
      visit?.(nextPath.join("."), value);
    } else if (value && typeof value === "object") {
      walkDictionary(value as Dictionary, nextPath, visit);
    }
  });
}

if (process.env.NODE_ENV !== "production") {
  const mojibakePattern = /(?:Ã.|Â.|�)/;
  const mojibakeHits: string[] = [];
  walkDictionary(dictionaries.EN, [], (path, value) => {
    if (mojibakePattern.test(value)) mojibakeHits.push(`EN:${path}`);
  });
  walkDictionary(dictionaries.ES, [], (path, value) => {
    if (mojibakePattern.test(value)) mojibakeHits.push(`ES:${path}`);
  });
  if (mojibakeHits.length) {
    throw new Error(
      `Mojibake detected in translations: ${mojibakeHits.join(", ")}`,
    );
  }
}

function getNestedValue(obj: unknown, key: string): unknown {
  return key.split(".").reduce<unknown>((acc, part) => {
    if (!acc || typeof acc !== "object") return undefined;
    return (acc as Record<string, unknown>)[part];
  }, obj);
}

function interpolate(template: string, vars?: Vars): string {
  if (!vars) return template;
  return template.replace(/\{(\w+)\}/g, (_, name: string) => {
    const value = vars[name];
    if (value === null || value === undefined) return "";
    return String(value);
  });
}

export function createTranslator(language: Language) {
  const dict = dictionaries[language] ?? dictionaries.EN;
  return (key: string, vars?: Vars) => {
    let raw = getNestedValue(dict, key);
    const missingInLocale = typeof raw !== "string";
    if (missingInLocale) {
      raw = getNestedValue(dictionaries.EN, key);
    }
    const missingEverywhere = typeof raw !== "string";

    if (missingEverywhere) {
      if (process.env.NODE_ENV !== "production") {
        // eslint-disable-next-line no-console
        console.warn(`Missing translation for key "${key}" in all locales.`);
      }
      return `${MISSING_TRANSLATION_PREFIX}${key}`;
    }

    if (missingInLocale && process.env.NODE_ENV !== "production") {
      // Surface missing locale entries early during development.
      // eslint-disable-next-line no-console
      console.warn(`Missing ${language} translation for key "${key}", using EN fallback.`);
    }

    return interpolate(raw as string, vars);
  };
}

